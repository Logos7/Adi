// =============================================================================
// brahma_bija_core.v
// Brahma-Bija processor core — multicycle FSM.
//
// Verilog-2001 / Gowin-safe version:
// - no logic / always_ff / always_comb / enum / int,
// - no SystemVerilog mode required in Project Configuration,
// - CADD/CSUB/CMUL/CABS2 are assembler macros; RTL only sees scalar ops,
// - call/return use a small hardware return stack: 16 x 32-bit.
// =============================================================================
...
module brahma_bija_core (
    ...
    // The bootloader writes program RAM without rebuilding the bitstream.
    // During this write, the top/bootloader keeps the core in reset.
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // UART RX has a one-byte buffer.
            // If the program does not read @uart_rx fast enough,
            // the new byte overwrites the previous one — simple and intentional at this stage.
                        ...
                        case (opcode)
                            ...
                            OP_RETURN: begin
                                if (return_sp == 5'd0) begin
                                    ...
                                    return_sp <= return_sp - 5'd1;
                                    pc <= return_stack[return_sp[3:0] - 4'd1];
                                    state <= S_FETCH;
                                end
                            end
