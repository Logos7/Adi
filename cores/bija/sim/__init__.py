"""Brahma-Bija v1.2 — prosty symulator CPU."""
...
ALU_FUNCT_NAMES = {
    FUNCT_IADD: "iadd", FUNCT_FADD: "fadd", FUNCT_ISUB: "isub", FUNCT_FSUB: "fsub",
    FUNCT_IAND: "iand", FUNCT_IOR: "ior", FUNCT_IXOR: "ixor", FUNCT_INOT: "inot",
    FUNCT_IMUL: "imul", FUNCT_FMUL: "fmul", FUNCT_ITOF: "itof", FUNCT_FTOI: "ftoi",
    FUNCT_SHL: "shl", FUNCT_SHR: "shr", FUNCT_SAR: "sar", FUNCT_FABS: "fabs", FUNCT_MOV: "move",
}
CMP_FUNCT_NAMES = {FUNCT_CMP_EQ: "cmp.eq", FUNCT_CMP_NE: "cmp.ne", FUNCT_CMP_LT: "cmp.lt", FUNCT_CMP_LE: "cmp.le", FUNCT_CMP_GT: "cmp.gt", FUNCT_CMP_GE: "cmp.ge"}
BOOL_FUNCT_NAMES = {FUNCT_BAND: "band", FUNCT_BOR: "bor", FUNCT_BXOR: "bxor", FUNCT_BNOT: "bnot", FUNCT_BMOV: "move"}
...
def field_opcode(word): return (word >> 26) & 0x3F
def field_pred(word): return word & 0xF
def field_rd(word): return (word >> 21) & 0x1F
def field_rs(word): return (word >> 16) & 0x1F
def field_rt(word): return (word >> 11) & 0x1F
def field_funct(word): return (word >> 4) & 0x7F
def field_md_imm(word): return (word >> 4) & 0x1FFFF
...
def field_ib_bd(word): return (word >> 22) & 0xF
def field_ib_bs(word): return (word >> 18) & 0xF
def field_ib_imm(word): return (word >> 4) & 0x3FFF
...
class CPU:
    ...
    def read_bool(self, num: int) -> int:
        if 0 <= num <= 7: return self.bool_regs[num]
        if num == 8: return 0
        if num == 9: return 1
    ...
    def step(self):
        ...
        if opcode == OPCODE_NOP:
            ...
            self._exec_alu_r(word); self.pc += 1
        elif opcode == OPCODE_CMP_R:
            self._exec_cmp_r(word); self.pc += 1
        elif opcode == OPCODE_LOAD_I:
            self.write_register(field_rd(word), field_i_imm(word)); self.pc += 1
            ...
            if (addr & 0xFF) == UART_TX_ADDR and not self.uart_tx_ready:
                self.cycle_count += 1
                return
            self.write_data_mem(addr, self.read_register(field_rd(word))); self.pc += 1
        elif opcode == OPCODE_LOAD_MD:
            self.write_register(field_rd(word), self.read_data_mem(field_md_imm(word))); self.pc += 1
            ...
            self.write_bool(field_ib_bd(word), self.read_bool_mem(field_ib_imm(word))); self.pc += 1
        elif opcode == OPCODE_SAVE_BI:
            self.write_bool_mem(field_ib_imm(word), self.read_bool(field_ib_bs(word))); self.pc += 1
        elif opcode == OPCODE_LOAD_BR:
            self.write_bool(field_rd(word), self.read_bool_mem(self.read_register(field_rs(word)) & 0x3FFF)); self.pc += 1
        elif opcode == OPCODE_SAVE_BR:
            self.write_bool_mem(self.read_register(field_rs(word)) & 0x3FFF, self.read_bool(field_rd(word) & 0xF)); self.pc += 1
        elif opcode == OPCODE_BOOL_R:
            self._exec_bool_r(word); self.pc += 1
        elif opcode == OPCODE_WAIT:
            self.wait_remaining = self.read_register(field_rs(word)); self.pc += 1
    ...
    def _exec_alu_r(self, word):
        ...
        sh = rt & 0x1F
        if funct == FUNCT_IADD or funct == FUNCT_FADD: result = a + b
        elif funct == FUNCT_ISUB or funct == FUNCT_FSUB: result = a - b
        elif funct == FUNCT_IAND: result = a & b
        elif funct == FUNCT_IOR: result = a | b
        elif funct == FUNCT_IXOR: result = a ^ b
        elif funct == FUNCT_INOT: result = ~a
        elif funct == FUNCT_IMUL: result = to_signed32(a) * to_signed32(b)
        elif funct == FUNCT_FMUL: result = fmul_q7_25(a, b)
        elif funct == FUNCT_ITOF: result = to_signed32(a) << 25
        elif funct == FUNCT_FTOI: result = to_signed32(a) >> 25
        elif funct == FUNCT_FABS: result = -to_signed32(a) if to_signed32(a) < 0 else to_signed32(a)
        elif funct == FUNCT_MOV: result = a
        elif funct == FUNCT_SHL: result = a << sh
        elif funct == FUNCT_SHR: result = a >> sh
        elif funct == FUNCT_SAR: result = to_signed32(a) >> sh
        else: raise RuntimeError(f"Nieznany funct {funct:#x} dla ALU")
    ...
    def _exec_bool_r(self, word):
        ...
        if funct == FUNCT_BAND: result = a & b
        elif funct == FUNCT_BOR: result = a | b
        elif funct == FUNCT_BXOR: result = a ^ b
        elif funct == FUNCT_BNOT: result = 1 - a
        elif funct == FUNCT_BMOV: result = a
        else: raise RuntimeError(f"Nieznany funct {funct:#x} dla BOOL")
    ...
    def _exec_cmp_r(self, word):
        ...
        if funct == FUNCT_CMP_EQ: result = a == b
        elif funct == FUNCT_CMP_NE: result = a != b
        elif funct == FUNCT_CMP_LT: result = sa < sb
        elif funct == FUNCT_CMP_LE: result = sa <= sb
        elif funct == FUNCT_CMP_GT: result = sa > sb
        elif funct == FUNCT_CMP_GE: result = sa >= sb
        else: raise RuntimeError(f"Nieznany funct {funct:#x} dla CMP")
    ...
    def dump_state(self):
        lines = [f"PC = {self.pc} (cycle = {self.cycle_count})"]
...
def disassemble(word: int, second_word: int = None) -> str:
    ...
    if opcode == OPCODE_NOP: return pred + "nop"
    if opcode == OPCODE_HALT: return pred + "halt"
    ...
    return f"{pred}<unknown opcode {opcode:#x}>"
