...
- cadd/csub/cmul/cabs2 oraz branch/wait/min/max jako makra assemblera.
"""
...
def validate_static_bool_access(addr: int, op: str):
    # Aktualny RTL implementuje bool_mem[0..127]. Adresy 128..129 są tylko odczytowym UART_READY/UART_RX_READY.
    ...
    raise AssemblerError(
        ...
        + (" oraz odczyt @uart_ready/@uart_rx_ready" if op == "load" else "")
    )
...
def expand_macro_line(line: str, counter: int):
    ...
    if m == "WAIT_RX":
        ...
        counter += 1
        return [f"{label}:", "move b7, @uart_rx_ready", f"(b7) jump {done}", f"jump {label}", f"{done}:"], counter
    ...
    if m == "READ_RX":
        ...
        counter += 1
        return [f"{label}:", "move b7, @uart_rx_ready", f"(b7) jump {done}", f"jump {label}", f"{done}:", f"move {ops[0]}, @uart_rx"], counter
    ...
    if m == "WAIT_UART":
        ...
        counter += 1
        return [f"{label}:", "move b7, @uart_ready", f"(b7) jump {done}", f"jump {label}", f"{done}:"], counter
    ...
    if m == "WRITE_TX":
        ...
        counter += 1
        return [f"{label}:", "move b7, @uart_ready", f"(b7) jump {done}", f"jump {label}", f"{done}:", f"move @uart_tx, {ops[0]}"], counter
        ...
        if line:
            ...
            for expanded_line in expanded:
                if expanded_line.strip():
                    raw_lines.append((lineno, expanded_line.strip()))
    labels = {}
    pending = []
    address = 0
