...
This variant keeps the working Agni UART bootloader path and adds an HDMI scanout path driven by a double-buffered dual-clock 1bpp framebuffer.

Display modes are selected with the board key:

- `0` - Agni external 320x200 framebuffer; `fbclear` / `fbplot` / `fberase` draw into the back buffer, and `fbswap` presents it on HDMI frame start
...
- `examples/agni/hdmi/wire_cube_hdmi.sutra`

These use `fbsize 320, 200` and do not transmit UART frames.
